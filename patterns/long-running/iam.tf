...
resource "aws_iam_role" "lambda_exec" {
  name               = "${var.name_prefix}-job-lambda"
  description        = "Execution role for the long-running pattern Lambda functions."
...
data "aws_iam_policy_document" "lambda_exec" {
  ...
  # The handlers read and write a single job record per execution.
  statement {
    sid    = "JobStateTable"
    effect = "Allow"
    actions = [
...
data "aws_iam_policy_document" "sfn_assume" {
  statement {
    ...
    # Confused-deputy protection: only allow assumption on behalf of resources
    # in this account.
    condition {
      test     = "StringEquals"
...
resource "aws_iam_role" "sfn_exec" {
  name               = "${var.name_prefix}-job-sfn"
  description        = "Execution role for the long-running pattern state machine."
  assume_role_policy = data.aws_iam_policy_document.sfn_assume.json
  tags               = local.tags
}

data "aws_iam_policy_document" "sfn_exec" {
  # Invoke exactly the four job functions — no wildcards.
  statement {
    sid       = "InvokeJobFunctions"
    effect    = "Allow"
    actions   = ["lambda:InvokeFunction"]
    resources = [for fn in aws_lambda_function.job : fn.arn]
  }
